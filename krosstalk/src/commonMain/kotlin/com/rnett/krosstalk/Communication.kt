...
internal suspend inline fun <T, K, reified C : ClientScope<*>> K.call(methodName: String, arguments: Map<String, *>): T
    ...
    val serializedUrlArgs = method.urlArguments(arguments)
        .mapValues { method.serializers.transformedParamSerializers.serializeArgumentToString(it.key, it.value).httpEncode() }
    ...
    if (method.useExplicitResult) {
        ...
        return when (result) {
            ...
            is InternalKrosstalkResponse.Success -> method.serializers.transformedResultSerializer.deserializeFromBytes(result.data) as T
