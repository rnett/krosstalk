...
class IrTryBuilder(private val builder: IrBuilderWithScope) {
    ...
    fun irCatch(throwableType: IrType, body: IrBuilderWithScope.(IrVariable) -> IrExpression) {
        ...
        if (!throwableType.isSubtypeOf(builder.context.irBuiltIns.throwableType,
                builder.context.irBuiltIns) && throwableType != builder.context.irBuiltIns.throwableType
        )
        ...
        val catchVariable =
            buildVariable(builder.scope.getLocalDeclarationParent(),
                builder.startOffset,
                builder.endOffset,
    ...
    @PublishedApi
    internal fun build(result: IrExpression, type: IrType): IrTry = builder.irTry(result, type, catches, finallyExpression)
}

inline fun IrBuilderWithScope.irTry(result: IrExpression, type: IrType = result.type, catches: IrTryBuilder.() -> Unit) =
